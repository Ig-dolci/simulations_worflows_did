...
frequency_peak = 9.0  # The dominant frequency of the Ricker wavelet.
...
c_true = Function(V).interpolate(2. + 1 * tanh(200 * (0.125 - sqrt((x - 0.5) ** 2 + (z - 0.5) ** 2))))
...
c_optimised = minimize(J_hat, method="L-BFGS-B", options={"disp": True, "maxiter": 50},
                        bounds=(1.5, 3.), derivative_options={"riesz_representation": "l2"})

VTKFile("c_optimised.pvd").write(c_optimised)

# with stop_annotating():
#     def gradient_descendent(J_hat, c_guess):
#         maxiter = 10
#         alpha0 = 0.01
#         J = 0.0
#         grad_file = VTKFile("grad.pvd")
#         c_guess_file = VTKFile("c_guess.pvd")
#         dJdc = Function(V)
#         for i in range(maxiter):
#             J0 = J
#             J = J_hat(c_guess)
#             print(f"Iteration {i}")
#             print(f"Functional value: {J} and J0: {J0}")
#             dJdc.assign(J_hat.derivative())
#             alpha = alpha0/(1. + i ** 0.5/10.0)
#             print(f"Alpha: {alpha}")
#             c_guess.assign(c_guess - alpha * dJdc)
#             c_guess_file.write(c_guess)
#             grad_file.write(dJdc)

#     gradient_descendent(J_hat, c_guess)
